...
from darksirenpop.likelihood import get_dz_and_jacobian
...
d_prior = [0, 10]
fagn_prior = [0, 1]
UNIFORM_PRIOR = np.array([b_prior, c_prior, d_prior, fagn_prior])
...
def log_prior_gauss(theta):
    ...
    logp_gauss = -0.5 * np.sum(((gauss_params - PRIOR_MU) / PRIOR_SIGMA) ** 2 + np.log(2 * np.pi * PRIOR_SIGMA**2))
...
def log_prior_uniform(theta):
    ...
    for i in range(len(theta)):
        if theta[i] < UNIFORM_PRIOR[i, 0] or theta[i] > UNIFORM_PRIOR[i, 1]:
            return -np.inf
    return 0.0


def init_uniform_sampler(filename, nwalkers):
    backend = emcee.backends.HDFBackend(filename)  # Run backend.reset(nwalkers, ndim) to create empty new file
...
def init_gaussian_sampler(filename, nwalkers):
    backend = emcee.backends.HDFBackend(filename)  # Run backend.reset(nwalkers, ndim) to create empty new file
...
def log_likelihood(theta, LTHRESH_STRING, GW_EVIDENCE_DICT, return_spline=False):
    agn_zcut = 3.0
    b, c, d, fagn = theta
    ...
    cfg.REAL_DATA = True
    cfg.OUTFILE = "none"
    cfg.AGN_DIST_DIR = AGN_DIST_DIR
    cfg.CATALOG_PATH = QUAIA_PATH
    cfg.ZMIN = 0.000001
    cfg.ZMAX = 10
    cfg.AGN_ZMAX = 10
    ...
    if return_spline:
        return interped_log_llh
    else:
        return interped_log_llh(fagn)


def log_probability(theta, log_prior, LTHRESH_STRING, GW_EVIDENCE_DICT, return_spline):
    lp = log_prior(theta)
    if not np.isfinite(lp):
        return -np.inf
    return lp + log_likelihood(theta, LTHRESH_STRING, GW_EVIDENCE_DICT, return_spline)
...
def main():
    ...
    if PRIOR == "uniform":
        ...
        backend, pos = init_uniform_sampler(filename=f"{GENERATED_DATA_DIR}/alt_origin_hyperparameters/sampler_uniform_priors_withcat_{LTHRESH_STRING}.h5", nwalkers=nwalkers)
    elif PRIOR == "gaussian":
    ...
    with Pool(processes=ncores, maxtasksperchild=50) as pool:
        sampler = emcee.EnsembleSampler(nwalkers, ndim, log_probability, pool=pool, backend=backend,
                                        args=(log_prior, LTHRESH_STRING, GW_EVIDENCE_DICT, False))
