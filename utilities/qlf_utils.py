...
from darksirenpop.utilities.utils import log10addexp10
...
def numdens2pdf(n_of_z: callable, redshift: np.ndarray, cosmo=COSMO, dV_dzdOmega_precompute=None) -> np.ndarray:
    '''
    ...
    if dV_dzdOmega_precompute is not None:
        dV_dzdOmega = dV_dzdOmega_precompute
    else:
        dV_dzdOmega = cosmo.differential_comoving_volume(redshift).value
    
    agn_redshift_pdf = 4 * np.pi * n_of_z(redshift) * dV_dzdOmega * np.diff(redshift)[0]
