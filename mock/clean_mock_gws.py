...
TYPES = ['agn', 'alt']
DIRECTORY_IDS = np.arange(65, 100, 1)
...
def evaluate_skymap(filename, completeness_map=COMPLETENESS_MAP, skymap_cl=SKYMAP_CL):
    ...
    print(f'Total probability in selected pixels: {np.sum(dP[skyprob_nonzero & pixprob_within_cl])}')

    if np.isinf(np.median(mu[skyprob_nonzero & pixprob_within_cl])):

        # median = np.median(mu[skyprob_nonzero & pixprob_within_cl])
        print(filename, 'inf caught')

        # print(median, 'aaa')
        # print(np.sum(np.isinf(mu[skyprob_nonzero & pixprob_within_cl])), len(mu))
        # print(sigma[skyprob_nonzero & pixprob_within_cl][np.isinf(mu[skyprob_nonzero & pixprob_within_cl])])
        # print(norm[skyprob_nonzero & pixprob_within_cl][np.isinf(mu[skyprob_nonzero & pixprob_within_cl])])
        # sys.exit(1)

    elif np.median(mu[skyprob_nonzero & pixprob_within_cl]) > 0:
    ...
    # print(np.sum(dP[skyprob_nonzero & pixprob_within_cl][ np.isinf(mu[skyprob_nonzero & pixprob_within_cl]) ]), 'total dP of those pixels in mu=inf pixels')
    # print(f'IF we decide to remove mu=inf pixels, we are left with total dP={np.sum(dP[skyprob_nonzero & pixprob_within_cl][ ~np.isinf(mu[skyprob_nonzero & pixprob_within_cl]) ])}')
    ...
    if np.sum(dP[skyprob_nonzero & pixprob_within_cl][ ~np.isinf(mu[skyprob_nonzero & pixprob_within_cl]) ]) < 0.998:
        print(f'Warning for event {filename}: dP in pixels with finite mu is {np.sum(dP[skyprob_nonzero & pixprob_within_cl][ ~np.isinf(mu[skyprob_nonzero & pixprob_within_cl]) ])}')
...
if REAL_DATA:
    ...
    SKYMAP_JSON_PATH = '/home/lucas/Documents/PhD/gw_data/reweighted-gwtc5/real_skymaps_reweight_gwtc5.json'  # json that contains paths to skymaps
    SKYMAP_EVALS_JSON_PATH = '/home/lucas/Documents/PhD/gw_data/reweighted-gwtc5/real_skymaps_evaluated.json'  # json that contains paths to redshift posteriors
    SKYMAP_CW_EVALS_JSON_PATH = '/home/lucas/Documents/PhD/gw_data/reweighted-gwtc5/real_cw_skymaps_evaluated.json'  # json that contains paths to completeness-weighted redshift posteriors
    
    WRITE_DIR = '/home/lucas/Documents/PhD/gw_data/reweighted-gwtc5/real_skymaps_evaluated/'  # path to all real-data redshift posteriors
    ...
    for key in skymaps_dict.keys():
        ...
        # if key in low_snr_events:
        #     continue
            # print(f'Low SNR event: {key}')
