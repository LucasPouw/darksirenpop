import numpy as np
# np.random.seed(26011996)  # For reproducibility
...
####################################################################
# For simulations:
DIST_ERR_DIVBY_RMAX = 0.2  # Delta R / Rmax
RMAX_GW = 1
RMIN = 0
VOLUME = 4 / 3 * np.pi * RMAX_GW**3
AGN_NUMDENS = int(1e3) / VOLUME
BATCH = int(100)
N_MC_SAMPS = int(1e4)
N_TRIALS = 10000
...
fname = 'posteriors_replaceFalse_relerr0.2_1kAGN_trunc27'
...
# n_integrals = 1000
# pos = np.linspace(RMIN, RMAX_GW, n_integrals)
# results = np.zeros(n_integrals)
# print('Precalculating possible p_alt...')
# for i, obs_loc in tqdm( enumerate(pos) ):
#     func = lambda x: dVdr_prior(x, RMAX_GW) * truncnorm.pdf(x, a=(RMIN - obs_loc) / GW_DIST_ERR, b=(RMAX_GW - obs_loc) / GW_DIST_ERR, loc=obs_loc, scale=GW_DIST_ERR)
#     results[i] = quad(func, RMIN, RMAX_GW)[0]
# p_alt_interp = interp1d(pos, results)


PHI = lambda x: norm.cdf(x, loc=0, scale=1)  # standard normal CDF
def integral_truncnorm_times_prior(mu, sig=GW_DIST_ERR, rmin=RMIN, rmax=RMAX_GW):
    '''
...
def process_gw_batch(obs_pos_gw, agn_pos, batch=BATCH, gw_dist_err=GW_DIST_ERR, rmin=RMIN, rmax=RMAX_GW, n_mc_samps=N_MC_SAMPS):
    '''Currently tested for batch size of 100 GWs. Performance could be worse for different batch sizes.'''

    posterior = lambda x: truncnorm.pdf(x, a=(rmin - obs_pos_gw) / gw_dist_err, b=(rmax - obs_pos_gw) / gw_dist_err, loc=obs_pos_gw, scale=gw_dist_err)
    p_agn = np.sum( posterior(agn_pos[:,np.newaxis]), axis=0 ) / len(agn_pos)

    # p_alt = np.zeros_like(p_agn)
    # for i in range(n_mc_samps // int(1e4)):
    #     samps = truncnorm.rvs(a=(rmin - obs_pos_gw) / gw_dist_err, b=(rmax - obs_pos_gw) / gw_dist_err, loc=obs_pos_gw, scale=gw_dist_err, size=(int(1e4), batch))
    #     p_alt += np.sum( dVdr_prior(samps, rmax), axis=0)  # NORMALIZE UP TO RMAX_GW, since p_rate(r > RMAX_GW) = 0
    # p_alt /= n_mc_samps
    # r_pos = ( np.random.uniform(size=(n_mc_samps, batch), low=0.0, high=rmax**3) )**(1/3)
    # p_alt_r_pos = np.sum( posterior(r_pos), axis=0 ) / len(r_pos)
    # print("difference between p_alt and p_alt_r_pos: ", np.max(np.abs(p_alt - p_alt_r_pos)/p_alt_r_pos))
    # breakpoint()

    p_alt = integral_truncnorm_times_prior(obs_pos_gw)

    
    # p_alt = np.zeros_like(p_agn)
    # for i, obs_pos in enumerate(obs_pos_gw):
    #     func = lambda x: truncnorm.pdf(x, a=(0 - obs_pos) / gw_dist_err, b=(rmax - obs_pos) / gw_dist_err, loc=obs_pos, scale=gw_dist_err) * dVdr_prior(x, rmax)

    #     p_alt[i] = quad(func, 0, rmax)[0]

    #     # print((p_alt_mc[i] - p_alt[i]) / p_alt[i])

    #     # print((p_alt_itp[i] - p_alt[i]) / p_alt[i])
...
#     obs_pos_gw_agn = truncnorm.rvs(a=(rmin - true_pos_gw_agn) / gw_dist_err, b=(rmax - true_pos_gw_agn) / gw_dist_err, loc=true_pos_gw_agn, scale=gw_dist_err)
#     obs_pos_gw_alt = truncnorm.rvs(a=(rmin - true_pos_gw_alt) / gw_dist_err, b=(rmax - true_pos_gw_alt) / gw_dist_err, loc=true_pos_gw_alt, scale=gw_dist_err)
...
#     log_llh_numerator_per_event = np.log(fagn_times_fobsc * total_cw_prob_agn + (1 - fagn_times_fobsc) * total_cw_prob_alt)
...
#     # Plot log-likelihood
#     # plt.figure(); plt.plot(log_llh_x_ax, log_llh, '.'); plt.show()
#     # print(index, log_llh_x_ax[np.argmax(log_llh, axis=0)])

#     if np.sum(np.isinf(log_llh)) != 0:
#         return index, np.zeros_like(log_llh)  # Something went wrong, ignore this trial

#     return index, log_llh  # log_llh_x_ax[np.argmax(log_llh, axis=0)]  # Maximize along logllh axis - TODO: change to interpolation




def generate_and_process_universe_realization(fagn, rmin=RMIN, rmax=RMAX_GW, nagn=NAGN, batch=BATCH, gw_dist_err=GW_DIST_ERR):
    # GENERATE AGN CATALOG
    agn_pos, _, _ = uniform_shell_sampler(rmin, rmax, nagn)  # NO AGN NEEDED ABOVE RMAX_GW, p_rate = 0
    ...
    # GENERATE GWS UP TO RMAX_GW
    true_pos_gw_agn = np.random.choice(agn_pos, n_agn_events, replace=False)
    true_pos_gw_alt, _, _ = uniform_shell_sampler(rmin, rmax, batch - n_agn_events)
    ...
    # CALCULATE HYPOTHESIS EVIDENCE
    obs_pos_gw = np.hstack((obs_pos_gw_agn, obs_pos_gw_alt))

    return process_gw_batch(obs_pos_gw, agn_pos) 
...
if __name__ == '__main__':
    ...
    # # print(estimation_arr[:,-10])
    # fagn_medians = np.median(estimation_arr, axis=0)
    # fagn_mean = np.mean(estimation_arr, axis=0)
    # q016 = np.quantile(estimation_arr, 0.16, axis=0)
    # q084 = np.quantile(estimation_arr, 0.84, axis=0)

    # plt.figure(figsize=(8,8))
    # plt.plot(TRUE_FAGNS, fagn_medians, color='red', linewidth=3, label='Median')
    # plt.plot(TRUE_FAGNS, fagn_mean, color='blue', linewidth=3, label='Mean')
    ...
    # plt.plot(TRUE_FAGNS, q016, linestyle='dotted', color='red', zorder=5, linewidth=3)
    # plt.plot(TRUE_FAGNS, q084, linestyle='dotted', color='red', zorder=5, linewidth=3, label=r'$68\%$ CI')
    # plt.legend()
